...
Describe "PSScriptAnalyzer rule-sets" -Tag Build , ScriptAnalyzer {
    ...
    # only pass module functions into pester framework
    $scripts = Get-ChildItem $ModuleBase -Include *.ps1 -Recurse |
        ? {
            ($_.FullName -match 'functions') -or ($_.FullName -match 'internal')
        }
